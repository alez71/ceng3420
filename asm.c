...
int inst_to_binary(
	...
	if (is_opcode(opcode) == ADDI) {
        ...
        binary += (0x01 << 12);
        binary += (reg_to_num(arg2, line_no) << 15);
        binary += (MASK11_0(lower5bit(arg3, line_no)) << 20);
	...
	// Unconditional Jumps
        ...
        binary = 0x67;
        binary += (reg_to_num(arg1, line_no) << 7);
		struct_regs_indirect_addr *target = parse_regs_indirect_addr(arg2, line_no);
        binary += (reg_to_num(target->reg, line_no) << 15);
        binary += (target->imm << 20);
        ...
        //imm[19:12]
        binary += (((offset & 0x7F800) >> 11) << 12);
        //imm[11]
        binary += (((offset & 0x400) >> 10) << 20);
        //imm[10:1]
        binary += (((offset & 0x3FF) >> 1) << 21);
        //imm[20]
        binary += (((offset & 0x8000) >> 19) << 30);
	}
	...
	// Conditional Branches
        ...
        binary += (reg_to_num(arg1, line_no) << 15);
        binary += (reg_to_num(arg2, line_no) << 20);
        int offset = label_to_num(arg3, 12, label_table, cmd_no, line_no);
        // imm[11]
		binary += ((offset & 0x800) >> 4);
		// imm[4:1]
		binary += ((offset & 0x1E) << 7);
	...
	// Load and Store Instructions
        ...
        //imm[11:5]
        binary += (((MASK11_0(offset) & 0xFE0) >> 5) << 25);
